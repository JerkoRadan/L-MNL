...
def get_inverse_Hessian(model, model_inputs, labels, layer_name='Utilities'):
    ...
    #beta_gradient = K.gradients(model.loss, beta_layer.weights[0])[0]
    ...
    #input_tensors= model.inputs + model._feed_sample_weights + model._feed_targets + [K.learning_phase()]
...
def get_inputs_gradient(model, model_inputs, labels, inputs_indice=0):
    ...
    # Define the function
    input_tensors= model_dummy.inputs + model_dummy.trainable_weights[2] + model_dummy.outputs + [K.learning_phase()]
    #input_tensors= model_dummy.inputs + model_dummy.sample_weights + model_dummy.targets + [K.learning_phase()]
    
    #inputs_gradient = K.gradients(model_dummy.loss, inputs_layer_placeholder)
